...
Menubar.Edit = function ( editor ) {
	...
	title.setTextContent( 'Редактировать' );
	container.add( title );
	...
	undo.setTextContent( 'Отменить (Ctrl+Z)' );
	...
	redo.setTextContent( 'Повторить (Ctrl+Shift+Z)' );
	...
	option.setTextContent( 'Очистить историю' );
	option.onClick( function () {

		if ( confirm( 'История будет очищена. Вы уверены?' ) ) {
	...
	var option = new UI.Row();
	option.setClass( 'option' );
	option.setTextContent( 'Клонировать' );
	option.onClick( function () {
	...
	var option = new UI.Row();
	option.setClass( 'option' );
	option.setTextContent( 'Удалить (Del)' );
	option.onClick( function () {

		var object = editor.selected;

		if ( confirm( 'Удалить ' + object.name + '?' ) === false ) return;
	...
	var option = new UI.Row();
	option.setClass( 'option' );
	option.setTextContent( 'Минимизировать шейдеры' );
	option.onClick( function() {
			...
			if ( material instanceof THREE.ShaderMaterial ) {
				...
				try {
					...
					if ( e instanceof glslprep.SyntaxError )
						...
						errors.push( path +
								": Ошибка (подробности см. в консоли)." );
